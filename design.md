...
**モバイル（60rem 未満）ではナビ列の末尾に「連絡する」を出す。** レール下端の
固定表示は狭い画面で場所を食うため消しているが、そこで導線がゼロになるのは誤り。
採用担当は LinkedIn のアプリ内ブラウザ＝モバイル幅で開くので、
**最も濃い流入で一番大事な導線が落ちていた。**
リンク先の情報源は `formalContact` ひとつのまま、描画だけ2箇所にする。
...
行き先の並びは `src/config/site.ts` の `sections` が唯一の情報源で、
レール・フッター・404・トップの「これまで」の群がすべてこれに従う。

順は **採用担当が読む順**。経歴 → 技術 → OSS → 運営 → 執筆。
何をしてきたか → 何ができるか → コードの証拠 → 場づくり → 書いたもの。

以前は運営・OSS・経歴・技術・執筆で、経歴が5つ中3番目だった。
トップの CTA が「職務経歴書を読む」でサイトの第一目的も採用なのに、
**一番行ってほしい先がレールの真ん中にあった。**
サイドレールは上から読まれるので、先頭は最も行ってほしい先に使う。

トップの表の群も同じ順にする。同じページ内に順序が2つあると、
どちらが優先なのか読み手に伝わらない。
...
- 言語切替えはフッターにも1本置く。上部の `LocaleSwitch` は絶対配置なので、
  読み終えた位置からは見えない。切り替えるためだけに先頭へ戻らせない。
  `alternatePath` を `Base` から渡す（渡さないと 404 のフッターが
  `/en/404/` という存在しないパスを指す）
- 言語切替えは本文カラムの右上（`LocaleSwitch`）。レール下端に置いていたが、
  そこは最後までスクロールしないと目に入らない。切替えを探すのは「開いた瞬間に
  読めない」人なので、最初の画面に出す。`.page` を基準にした絶対配置にして、
  罫線の右端に揃える。グリッドの行にするとページ側の上余白が押し下げられる
