...
**ただし、この一文を看板（トップの見出し）に出してはならない。** 抽象的な標語を
トップに置く試みは過去に2回失敗しており、`design.md` の「看板に思想を書かない」で
禁止されている。看板は名前と読みだけにする。

差別化要因は文で主張せず、構造で示す。継続中の職歴を先頭に置き、在籍期間を帯で
重ねて見せれば、「同時にやっている」は読者が自分で読み取る。この節はあくまで
**編集方針**であって、ページに載せる文言ではない。
